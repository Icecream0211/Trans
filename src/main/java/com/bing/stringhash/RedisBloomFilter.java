...
public class RedisBloomFilter {
    ...
    public RedisBloomFilter(long expectedInsertions, double fpp, String host, int port) {
        this.expectedInsertions = expectedInsertions;
        this.fpp = fpp;
        numBits = optimalNumOfBits(expectedInsertions, fpp);
        numHashFunctions = optimalNumOfHashFunctions(expectedInsertions, fpp);
    ...
    public static void main(String[] args) {
        Charset charset = Charset.forName("UTF-8");
        String key = "2088002000270193";
        long hashKey =  Hashing.murmur3_128().hashObject(key, Funnels.stringFunnel(charset)).asLong();
